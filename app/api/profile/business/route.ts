...
import { DbAddress, DbBusiness, DbUsers } from "@/app/lib/models";
import { fetchBusinessInfoByUserId, getAddressInfo, fetchNextAddressId, fetchNextBusinessId } from "@/app/lib/data";
...
export async function GET(_req: NextRequest) {
    ...
    if (!bizz.businessInfo) {
        ...
        return NextResponse.json({ message: "Business not found" }, { status: 200 });
    }

    const address = await getAddressInfo(bizz.businessInfo.idAddress);
    if (!address.addressData) {
        //il est possible qu'un utilisateur n'ait pas encore inscrit son business
        return NextResponse.json({ message: "Business address not found" }, { status: 200 });
    }

    //on remet les informations sous le format BusinessSchema
    const res = NextResponse.json(toPublic(bizz.businessInfo, address.addressData), { status: 200 });
...
export async function PUT(req: NextRequest) {
    ...
    if (typeof businessNumber === "string") setBusiness.businessNumber = businessNumber;
    ...
    // Aucun changement
    if (Object.keys(setBusiness).length === 0 && Object.keys(setAddress).length === 0) {
    ...
    if (Object.keys(setAddress).length > 0) {
        if (finalAddressId && finalAddressId != -1) {
            // m.a.j.
            await DbAddress.findOneAndUpdate({ idAddress: finalAddressId }, { $set: setAddress });
        } else {
            // creation d'un nouvel id
            finalAddressId = await fetchNextAddressId();
            // creation d'une addresse dans la BD
            const newAddress = new DbAddress({ idAddress: finalAddressId, ...setAddress });
    ...
    if (!existing) {
        ...
        if (typeof newBizz.idBusiness === "number") setUser.idBusiness = newBizz.idBusiness;
    ...
    if (!updated) {
        return NextResponse.json({ message: "Erreur auprès de la recherche/creation de la compagnie." }, { status: 404 });
    }

    // fetch address data for response
    const addressRes = await getAddressInfo(updated.idAddress);
    const addressData = addressRes.addressData ?? { address: "", city: "", zipCode: "", province: "", country: "" };
