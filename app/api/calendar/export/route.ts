...
import { getAllFacturesUsers } from "@/app/_lib/database/queries";
import { validateCalendarToken } from "@/app/lib/calendar-token";
import {
  logCalendarAccess,
  detectSuspiciousActivity,
} from "@/app/lib/calendar-audit";
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (ENABLE_RATE_LIMITING) {
      ...
      if (lastAccess && now - lastAccess < RATE_LIMIT_WINDOW_MS) {
        const retryAfter = Math.ceil(
          (RATE_LIMIT_WINDOW_MS - (now - lastAccess)) / 1000,
        );
    ...
    typedInvoices.forEach((invoice: InvoiceData) => {
      ...
      const dtstamp = invoice.updatedAt
