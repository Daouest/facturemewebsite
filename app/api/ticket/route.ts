...
import { getAllTickets, updateTicket, deleteTicket } from "@/app/_lib/database/queries";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const lastTicketDate = result.ticket
      ? result.ticket?.[parseInt(count) - 1].date.toISOString()
    ...
    return NextResponse.json(
      { error: "Impossible d'avoir les ticktes" },
      { status: 500 },
    );
  }
}
