...
import { ConfirmQuery } from "@/app/lib/schemas/auth"
import { ObjectId } from "mongodb"
import { APP_URL } from "@/app/lib/schemas/env"

const redirect = (slug: "verified" | "link-invalid" | "link-expired" | "link-already-used") =>
...
export async function GET(req: Request) {
    ...
    const parsed = ConfirmQuery.safeParse({ token: url.searchParams.get("token") })
    if (!parsed.success) {
    ...
    //update dans la bd
    await db.collection("email_token").updateOne({ _id: doc._id }, { $set: { used: true, usedAt: new Date() } })
    await db.collection("users").updateOne({ _id: new ObjectId(doc.uid) }, { $set: { emailVerified: new Date() } })
