...
import { SignupSchema } from "@/app/_lib/schemas/auth";
import { APP_URL } from "@/app/_lib/schemas/env";
import { getNextId } from "@/app/_lib/database/sequence-generator";
...
export async function POST(req: Request) {
  console.log(
    `[SIGNUP] Starting signup process at ${new Date().toISOString()}`,
  );
  ...
  try {
    ...
    if (!parsed.success) {
      ...
      return NextResponse.json(
        { message: "Erreur dans la validation", errors },
        {
          status: 400,
        },
      );
    ...
    //check
    const { username, firstName, lastName, email, password, address } =
      parsed.data;
    ...
    const idAddress = await getNextId(DbAddress, 'idAddress');
    ...
    const idUser = await getNextId(DbUsers, 'idUser');
    ...
    console.log(
      "[SIGNUP] Address created successfully:",
      addressDoc._id.toString(),
    );

    // Send confirmation email
    try {
      const emailResponse = await fetch(
        new URL("/api/auth/send-confirmation", APP_URL),
        {
          method: "POST",
