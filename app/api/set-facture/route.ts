import { setFacture, getSession } from "@/app/_lib/session/session-node";
import { getFactureData } from "@/app/_lib/database/queries";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (factureDataResponse.facture.idUser !== userId) {
      console.warn(
        `Authorization failed: User ${userId} tried to access invoice ${factureId} owned by ${factureDataResponse.facture.idUser}`,
      );
      return NextResponse.json(
