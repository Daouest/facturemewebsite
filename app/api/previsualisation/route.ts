...
export async function GET(request: Request) {
  ...
  if (typeof factureId !== "number" || isNaN(factureId)) {
    return new Response(
      JSON.stringify({ error: "Invalid or missing factureId parameter" }),
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (factureData.facture.idUser !== userId) {
      console.warn(
        `Authorization failed in previsualisation: User ${userId} tried to access invoice ${factureId} owned by ${factureData.facture.idUser}`,
      );
      return new Response(
    ...
    if (
      factureData &&
      factureData.facture &&
      factureData.facture.idUser != null
    ) {
      //  (FIRST) -------- userInfos ------------------------------------------------------------------------------
      ...
      if (isBusiness === true) {
        ...
        if (userInfo && userInfo.idBusiness != null) {
          ...
          if (businessInfo && businessInfo.idAddress != null) {
            //info addresse
            const addresseInfoMsg = await getAddressInfo(
              businessInfo.idAddress,
            );
            const addressInfo = addresseInfoMsg.addressData;

            //info taxes
            if (businessInfo.TVHnumber)
              taxesnumbers.push({
                taxName: "TVH",
                taxNumber: businessInfo.TVHnumber,
              });
            if (businessInfo.TVPnumber)
              taxesnumbers.push({
                taxName: "TVP",
                taxNumber: businessInfo.TVPnumber,
              });
            if (businessInfo.TVQnumber)
              taxesnumbers.push({
                taxName: "TVQ",
                taxNumber: businessInfo.TVQnumber,
              });
            if (businessInfo.TVSnumber)
              taxesnumbers.push({
                taxName: "TVS",
                taxNumber: businessInfo.TVSnumber,
              });
        ...
        if (userInfo && userInfo.idAddress != null) {
          ...
          console.error(
            "Personal invoice error: User does not have a personal address set in their profile.",
          );
          console.error(
            "User needs to go to Profile page and set their personal address.",
          );
        }
      }
      ...
      if (factureData.facture.idClient != null) {
        const infoClientMsg = await fetchCustomerInfo(
          factureData.facture.idClient,
        );
    ...
    return new Response(
      JSON.stringify({ error: "Failed to fetch facture data" }),
      { status: 500 },
    );
  }
}
