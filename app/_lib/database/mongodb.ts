...
import { requireMongoEnv } from "@/app/_lib/schemas/env";
...
export async function connectToDatabase(): Promise<Mongoose> {
  ...
  if (!cached.promise) {
    ...
    cached.promise = mongoose
      .connect(MONGODB_URI, { dbName: MONGODB_DB_NAME, autoIndex })
      .then((m) => {
        if (NODE_ENV !== "production")
          console.log(`Connected to MongoDB (db="${m.connection.name}")`);
        return m;
      });
  }
