...
import {
  DbFacture,
  DbObjetFacture,
  DbTauxHoraireFacture
} from "@/app/_lib/database/models";
import {
  fetchNextFactureId,
  validateInvoiceNumber,
  fetchObjectsByIds,
  fetchHourlyRatesByIds
} from "@/app/_lib/database/queries";

import {
  getUserFromCookies
} from "../session/session-node";
import {
  validateFormData,
  createInvoiceDocuments,
  validateDocuments,
  convertToItemTable
} from "@/app/_lib/helpers/invoice-helpers";


export type InvoiceCreationFormState = {
    errors?: {
        customerId?: string[];        number?: string[];        businessId?: string[];        invoiceType?: string[];        dateType?: string[];        invoiceDate?: string[];        items?: {
 [itemIndex: number]: {
 [field: string]: string[] 
}
...
;    message?: string | null;    formData?: InvoiceForm
}
...
export async function createInvoice(
  prevState: InvoiceCreationFormState,
  formData: FormData
): Promise<InvoiceCreationFormState> {
  // Parse and validate form data first
  const { invoiceForm, errors: validationErrors } = validateFormData(formData);
  
  if (validationErrors && Object.keys(validationErrors).length > 0) {
    return errorResponse(
      "Missing or invalid fields, failed to create invoice",
      invoiceForm,
      validationErrors
    );
  }
  ...
  const productIds = invoiceForm.items
    .filter(item => item.type === 'product')
    .map(item => item.id);
  const hourlyIds = invoiceForm.items
  ...
  const factureNumber = invoiceForm.numberType === "custom" 
  ...
  try {
    ...
    revalidatePath('/homePage');    redirect('/homePage');
}
