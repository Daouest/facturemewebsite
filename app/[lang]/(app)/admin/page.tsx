...
export default function AdminPage() {
  ...
  const TickePage = React.lazy(() => import("@/app/components/Ticket"));
  const { showPage } = useSidebarContext();
  const { user } = useUser();
  const router = useRouter();

  useEffect(() => {
    if (user?.isAdmin == false) router.push(`/${lang}/dashboard`);
  }, [user, router, lang]);

  return (
    <div className="relative flex justify-center items-start bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 px-6 overflow-hidden">
      <Link
        href={`/${lang}/dashboard`}
