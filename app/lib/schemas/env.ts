...
export function requireMongoEnv() {
  ...
  if (missing.length) throw new Error(`Mongo not configured: ${missing.join(", ")}`);
  return { MONGODB_URI: e.MONGODB_URI!, MONGODB_DB_NAME: e.MONGODB_DB_NAME!, NODE_ENV: e.NODE_ENV };
}
...
export function requireGmailEnv() {
  ...
  if (missing.length) throw new Error(`Email not configured: ${missing.join(", ")}`);
  return {
...
export const APP_URL =
  ...
  (getServerEnv().VERCEL_URL ? `https://${getServerEnv().VERCEL_URL}` : "http://localhost:3000");
