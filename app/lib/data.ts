...
import {DbObjet, DbBusiness, DbTauxHoraire, DbClient, DbFacture, DbObjetFacture, DbFactureHoraire, DbUsers, DbAddress, DbTicket } from "@/app/lib/models";
...
export async function deleteTicket(id: number) {
    try {
        await DbTicket.deleteOne({ _id: id});
        const item = await DbTicket.findOne({ _id: id });
        if (!item) return { success: true }
