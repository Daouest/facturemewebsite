...
const styles = StyleSheet.create({
  ...
  title: { 
    fontSize: 22, 
    marginBottom: 5,
    color: "#0F172A", // slate-900 - professional dark
    letterSpacing: 0.5,
  },
  section: { 
    margin: 5, 
  ...
  addressBlock: { 
    ...
    backgroundColor: "#FFFFFF",
    borderLeftWidth: 3,
    borderLeftColor: "#0EA5E9", // sky-500 accent bar
  ...
  boldText: { 
    fontSize: 11,
    color: "#0EA5E9", // sky-500 for labels
    marginBottom: 4,
  },
  label: {
    fontSize: 9,
  ...
  tableRow: {
    ...
    marginBottom: 2,
    borderLeftWidth: 2,
    borderLeftColor: "#E0F2FE", // sky-100 subtle accent
    borderLeftStyle: "solid",
  },
  tableHeader: { 
    flexDirection: "row",
    backgroundColor: "#0EA5E9", // sky-500 brand color
    marginBottom: 5,
    paddingTop: 8,
    paddingBottom: 8,
    paddingLeft: 8,
  ...
  totalRow: {
    ...
    width: 200,
    borderTopWidth: 2,
    borderTopColor: "#0EA5E9", // sky-500 accent for emphasis
    borderTopStyle: "solid",
    paddingTop: 8,
    marginTop: 8,
    backgroundColor: "#E0F2FE", // sky-100 highlight
    padding: 8,
  },
  totalText: {
    fontSize: 13,
    color: "#0C4A6E", // sky-900 - strong emphasis
  },
  footer: {
    position: "absolute",
    bottom: 20,
    ...
    borderTopWidth: 2,
    borderTopColor: "#0EA5E9", // sky-500 brand accent
    borderTopStyle: "solid",
    paddingTop: 8,
    flexDirection: "row",
    justifyContent: "space-between",
    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          {invoiceData.user?.logo && (
            <Image
              src={invoiceData.user.logo}
              style={styles.logo}
            />
          )}
          <Text style={styles.title}>{translations.invoiceTitle}</Text>
        </View>
        <View style={styles.headerRight}>
          <Text style={{ fontSize: 9, color: "#0C4A6E" }}>No: {invoiceData.facture?.factureNumber}</Text>
          <Text style={{ fontSize: 9, color: "#475569" }}>Date: {invoiceData.facture?.date}</Text>
        </View>
      </View>
      ...
      <View style={{ flexDirection: "row", gap: 10, marginBottom: 10 }}>
        {invoiceData.user && (
          <View style={[styles.addressBlock, { flex: 1 }]}>
            <Text style={styles.boldText}>{translations.from}</Text>
            <Text style={{ fontSize: 10, color: "#334155" }}>{invoiceData.user.name}</Text>
            <Text style={{ fontSize: 9, color: "#64748B" }}>{invoiceData.user.address}</Text>
        ...
        {invoiceData.client && (
          <View style={[styles.addressBlock, { flex: 1 }]}>
            <Text style={styles.boldText}>{translations.to}</Text>
            <Text style={{ fontSize: 10, color: "#334155" }}>{invoiceData.client.name}</Text>
            <Text style={{ fontSize: 9, color: "#64748B" }}>{invoiceData.client.address}</Text>
            <Text style={{ fontSize: 9, color: "#64748B" }}>
        ...
        invoiceData.facture.colonnesHoraire.length > 0 && (
          <View style={styles.table}>
            ...
            <View style={[styles.tableRow, styles.tableHeader]}>
              <Text style={[styles.tableCell, styles.tableHeaderText]}>{translations.position}</Text>
              <Text style={[styles.tableCell, styles.tableHeaderText]}>{translations.ratePerHour}</Text>
              <Text style={[styles.tableCell, styles.tableHeaderText]}>{translations.hours}</Text>
              <Text style={[styles.tableCell, styles.tableHeaderText]}>{translations.total}</Text>
            </View>
      ...
      {invoiceData.facture?.colonnesUnitaires &&
        invoiceData.facture.colonnesUnitaires.length > 0 && (
          <View style={styles.table}>
            ...
            <View style={[styles.tableRow, styles.tableHeader]}>
              <Text style={[styles.tableCell, styles.tableHeaderText]}>{translations.product}</Text>
              <Text style={[styles.tableCell, styles.tableHeaderText]}>{translations.description}</Text>
              <Text style={[styles.tableCell, styles.tableHeaderText]}>{translations.quantity}</Text>
              <Text style={[styles.tableCell, styles.tableHeaderText]}>{translations.unitPrice}</Text>
              <Text style={[styles.tableCell, styles.tableHeaderText]}>{translations.total}</Text>
            </View>
      ...
      <View style={styles.summary}>
        ...
        <View style={styles.totalRow}>
          <Text style={styles.totalText}>{translations.total}:</Text>
      ...
      <View style={styles.footer} fixed>
        <Text style={styles.footerText}>{translations.footerText}</Text>
        <Text 
...
export default function PDFInvoice({ invoiceId, language = "fr" }: PDFInvoiceProps) {
  ...
  const translations = useMemo(() => ({
    ...
    productsServices: language === "fr" ? "Produits/Services" : "Products/Services",
    ...
    footerText: language === "fr" ? "Facture générée par FactureMe" : "Invoice generated by FactureMe",
  }), [language]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // generate blob
        const blob = await pdf(<InvoiceDocument invoiceData={data} translations={translations} />).toBlob();
        if (!mounted) return;
    ...
    <div style={{ 
        ...
        <>
          ...
          >
            ...
            >
              ...
              >
              <PDFDownloadLink
                document={<InvoiceDocument invoiceData={invoiceData} translations={translations} />}
                fileName={`facture-${
              ...
              <button
                ...
                title={language === "fr" ? "Aide pour le téléchargement" : "Download help"}
              >
                ?
              </button>
              ...
              {showDownloadHelp && (
                ...
                >
                  ...
                  <div style={{ color: "#cbd5e1", lineHeight: "1.5" }}>
                    {language === "fr" 
                      ? "Par défaut, le fichier sera téléchargé dans votre dossier Téléchargements."
                      : "By default, the file will be downloaded to your Downloads folder."}
                  </div>
                </div>
              )}
            </div>
      ...
      {isGenerating && (
        ...
        >
          ...
          <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 8, color: "#0ea5e9" }}>
            {language === "fr" ? "Génération du PDF..." : "Generating PDF..."}
          </div>
          <div style={{ fontSize: 13, opacity: 0.7, color: "#cbd5e1" }}>
            {language === "fr" 
